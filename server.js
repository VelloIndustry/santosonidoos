...
app.post('/api/contact', async (req, res) => {
  ...
  const emailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; background: #111; color: #e0e0e0; padding: 24px; border-radius: 8px;">
      ...
      <table style="width:100%; border-collapse: collapse;">
        <tr><td style="padding: 6px 0; color: #888;">Name</td><td style="padding: 6px 0; color: #f5f5f5;">${name}</td></tr>
        <tr><td style="padding: 6px 0; color: #888;">Email</td><td style="padding: 6px 0; color: #f5f5f5;">${email}</td></tr>
        <tr><td style="padding: 6px 0; color: #888;">Phone</td><td style="padding: 6px 0; color: #f5f5f5;">${phone || '—'}</td></tr>
        <tr><td style="padding: 6px 0; color: #888;">Service</td><td style="padding: 6px 0; color: #f5f5f5;">${service || '—'}</td></tr>
        <tr><td style="padding: 6px 0; color: #888;">City</td><td style="padding: 6px 0; color: #f5f5f5;">${city || '—'}</td></tr>
      </table>
      <div style="margin-top: 16px; border-top: 1px solid #333; padding-top: 16px;">
        ...
        <p style="white-space: pre-line; color: #f5f5f5;">${message}</p>
      </div>
    </div>`;
  ...
  const autoReplyHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; background: #111; color: #e0e0e0; padding: 24px; border-radius: 8px;">
      ...
      <p style="margin-bottom: 16px;">Hi <strong>${name}</strong>, thanks for reaching out to Santo Sonido OS.</p>
      <p style="margin-bottom: 16px;">We received your inquiry about <strong>${service || 'your project'}</strong> and will get back to you within 24 hours.</p>
